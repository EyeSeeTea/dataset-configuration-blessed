    ...
    function s(o, u) {
        ...
        (function ($) {
            ...
            $.feedback = function (options) {
                    ...
                    $(document).on('click', settings.feedbackButton, function () {
                        ...
                        $(document).on('mouseup', function () {
                            if (canDraw) {
                                ...
                                if (dtop + dheight > $(document).height()) dheight = $(document).height() - dtop;
                        ...
                        $(document).on('mousemove', function (e) {
                            if (canDraw && drag) {
                                ...
                                $('.feedback-helper').each(function () {
                                    if ($(this).attr('data-type') == 'highlight') drawlines(ctx, parseInt($(this).css('left'), 10), parseInt($(this).css('top'), 10), $(this).width(), $(this).height());
                                });
                                if (highlight == 1) {
                                ...
                                $('.feedback-helper').each(function () {
                                    if ($(this).attr('data-type') == 'highlight') ctx.clearRect(parseInt($(this).css('left'), 10), parseInt($(this).css('top'), 10), $(this).width(), $(this).height());
                                });
                                $('.feedback-helper').each(function () {
                            ...
                            $(document).on('mousemove click', '#feedback-canvas', function (e) {
                                if (canDraw) {
                                    ...
                                    $('* :not(body,script,iframe,div,section,.feedback-btn,#feedback-module *)').each(function () {
                                        if ($(this).attr('data-highlighted') === 'true') return;
                        ...
                        $(document).on('click', '#feedback-close', function () {
                            if (settings.highlightElement && $(this).parent().attr('data-highlight-id')) var _hidx = $(this).parent().attr('data-highlight-id');

                            $(this).parent().remove();

                            if (settings.highlightElement && _hidx) $('[data-highlight-id="' + _hidx + '"]').removeAttr('data-highlighted').removeAttr('data-highlight-id');
                ...
                function redraw(ctx, border) {
                    ...
                    ctx.fillRect(0, 0, $('#feedback-canvas').width(), $('#feedback-canvas').height());
                    $('.feedback-helper').each(function () {
                        if ($(this).attr('data-type') == 'highlight') if (border) drawlines(ctx, parseInt($(this).css('left'), 10), parseInt($(this).css('top'), 10), $(this).width(), $(this).height());
                    });
                    $('.feedback-helper').each(function () {
                        if ($(this).attr('data-type') == 'highlight') ctx.clearRect(parseInt($(this).css('left'), 10), parseInt($(this).css('top'), 10), $(this).width(), $(this).height());
                    });
        ...
        var EventHandler = function () {
            function EventHandler(dom_element) {
                ...
                if (this.dom_element) {
                    pointer_down_events.forEach(function (event) {
                        return _this.dom_element.addEventListener(event, _this, false);
                    });
                    pointer_move_events.forEach(function (event) {
                        return _this.dom_element.addEventListener(event, _this, false);
                    });
                    pointer_up_events.forEach(function (event) {
                        return _this.dom_element.addEventListener(event, _this, false);
                    });
                    resize_events.forEach(function (event) {
                        return _this.dom_element.addEventListener(event, _this, false);
                    });
                    transitionend_events.forEach(function (event) {
                        return _this.dom_element.addEventListener(event, _this, false);
                    });
                }
            }
            ...
            _createClass(EventHandler, [{
                ...
                key: "unbind_events",
                value: function unbind_events() {
                    var _this2 = this;

                    if (this.dom_element) {
                        pointer_down_events.forEach(function (event) {
                            return _this2.dom_element.removeEventListener(event, _this2, false);
                        });
                        pointer_move_events.forEach(function (event) {
                            return _this2.dom_element.removeEventListener(event, _this2, false);
                        });
                ...
                key: "on_pointer_events",
                value: function on_pointer_events(cb, event_type) {
                ...
                key: "off_pointer_events",
                value: function off_pointer_events(cb, event_type) {
                    if (event_type in this.cb.pointer) {
                        var index = this.cb.pointer[event_type].indexOf(cb);
                        var cb_exist = index !== -1;
                        if (cb_exist) {
                            this.cb.pointer[event_type].splice(index, 1);
                        }
                    } else {
                        throw "Unrecognized pointer event-type";
                    }
                ...
                key: "on_resize",
                value: function on_resize(cb) {
                    this.cb.resize.add(cb);
                    return this;
                }
            }, {
                key: "off_resize",
                value: function off_resize(cb) {
                    var index = this.cb.resize.indexOf(cb);
                    var cb_exist = index !== -1;
                    if (cb_exist) {
                        this.cb.resize.splice(index, 1);
                    }
                    return this;
                }
            }, {
                key: "on_transitionend",
                value: function on_transitionend(cb) {
                    this.cb.transitionend.add(cb);
                    return this;
                }
            }, {
                key: "off_transitionend",
                value: function off_transitionend(cb) {
                    var index = this.cb.transitionend.indexOf(cb);
                    var cb_exist = index !== -1;
                    if (cb_exist) {
                        this.cb.transitionend.splice(index, 1);
                    }
                    return this;
                }
            }, {
                key: "_pointer_handler",
                value: function _pointer_handler(e, event_type) {
                    if (event_type in this.cb.pointer) {
                        this.cb.pointer[event_type].forEach(function (cb) {
                            return cb(e);
                        });
                    } else {
                        throw "Unrecognized pointer event-type";
                ...
                key: "_resize_handler",
                value: function _resize_handler(e) {
                    this.cb.resize.forEach(function (cb) {
                        return cb(e);
                    });
                    return this;
                }
            }, {
                key: "_transitionend_handler",
                value: function _transitionend_handler(e) {
                    this.cb.transitionend.forEach(function (cb) {
                        return cb(e);
                    });
                    return this;
                }
